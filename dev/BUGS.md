...
**6 open** · 0 S1 · 0 S2 · 6 S3  |  **201 fixed**  |  **1 withdrawn** (B-096)
...
| **B-139** | S3 | v3.14.372 | **Decided, not built: the three string lists need no per-element provenance, and the bug had grouped them by the wrong property.** It asked about STORAGE SHAPE — string versus object — when the deciding property is the CONTROL. B-138 stamps elements because a row editor lets two people add rows years apart; these three are written by whole-value controls — one CSV input, one picker — so every element is one person's single act, and N identical stamps say less than one field stamp. `constituent_forms` is joined into a parse, so its order is the analysis |
